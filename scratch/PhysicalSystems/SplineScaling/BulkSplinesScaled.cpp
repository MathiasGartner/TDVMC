...
BulkSplinesScaled::BulkSplinesScaled(vector<double>& params, string configDirectory) :
...
void BulkSplinesScaled::InitSystem()
{
	grBinCount = 100;
	grBinStartIndex = 3;
	numOfkValues = 300;
...
vector<double> BulkSplinesScaled::GetCenterOfMass(vector<vector<double> >& R)
{
	vector<double> com = { 0.0, 0.0, 0.0 };
	return com;
}

double BulkSplinesScaled::GetExternalPotential(vector<double>& r)
{
	return 0;
}

void BulkSplinesScaled::CalculateOtherLocalOperators(vector<vector<double> >& R)
{

}

void BulkSplinesScaled::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	localEnergyI = kineticI + potentialInternComplex;
...
void BulkSplinesScaled::CalculateExpectationValues(ICorrelatedSamplingData* sample, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{

}

void BulkSplinesScaled::CalculateAdditionalSystemProperties(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	//int index = 0;
...
void BulkSplinesScaled::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double sum = 0;
	double interval;
	int bin;
...
void BulkSplinesScaled::CalculateWavefunction(ICorrelatedSamplingData* sample, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{

}

void BulkSplinesScaled::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	double sum = 0;
	double interval;
	int bin;
...
double BulkSplinesScaled::CalculateWFQuotient(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
...
void BulkSplinesScaled::AcceptMove()
{
	wf = wfNew;
...
void BulkSplinesScaled::InitCorrelatedSamplingData(vector<ICorrelatedSamplingData*>& data)
{
}

void BulkSplinesScaled::FillCorrelatedSamplingData(ICorrelatedSamplingData* data)
{
}
