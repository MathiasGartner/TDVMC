#include "BulkOnlySplinesOrigin.h"

BulkOnlySplinesOrigin::BulkOnlySplinesOrigin(string configDirectory) : IPhysicalSystem()
{
...
void BulkOnlySplinesOrigin::InitSystem()
{
	grBinCount = 100;
	grBinStartIndex = 3;
	numOfkValues = 300;
...
vector<double> BulkOnlySplinesOrigin::GetCenterOfMass(vector<vector<double> >& R)
{
	vector<double> com = {0.0, 0.0, 0.0};
	return com;
}

void BulkOnlySplinesOrigin::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	for (int n = 0; n < N; n++)
	{
		...
		for (int a = 0; a < DIM; a++)
		{
			temp = (bcFactors[0][0] * splineSumsD[0][n][a] + bcFactors[0][1] * splineSumsD[1][n][a] + bcFactors[0][2] * splineSumsD[2][n][a]);
		...
		temp = (bcFactors[0][0] * splineSumsD2[0][n] + bcFactors[0][1] * splineSumsD2[1][n] + bcFactors[0][2] * splineSumsD2[2][n]);
		kineticSumR2 += uR[0] * temp;
		kineticSumI2 += uI[0] * temp;
		for (int k = 1; k < numberOfStandardParameters; k++)
		{
			for (int a = 0; a < DIM; a++)
			{
				vecKineticSumR1[a] += uR[k] * splineSumsD[k+2][n][a];
				vecKineticSumI1[a] += uI[k] * splineSumsD[k+2][n][a];
			}
			kineticSumR2 += uR[k] * splineSumsD2[k+2][n];
			kineticSumI2 += uI[k] * splineSumsD2[k+2][n];
		}
		for (int a = 0; a < DIM; a++)
		{
	...
	localOperators[0] = (bcFactors[0][0] * splineSums[0] + bcFactors[0][1] * splineSums[1] + bcFactors[0][2] * splineSums[2] +bcFactors[0][3]);
	for (int i = 1; i < numberOfStandardParameters; i++)
	{
		localOperators[i] = splineSums[i+2];
	}
...
void BulkOnlySplinesOrigin::CalculateAdditionalSystemProperties(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
    ClearVector(additionalSystemProperties);
...
void BulkOnlySplinesOrigin::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double sum = 0;
	double interval;
	int bin;
	...
	sum += uR[0] * (bcFactors[0][0] * splineSums[0] + bcFactors[0][1] * splineSums[1] + bcFactors[0][2] * splineSums[2] + bcFactors[0][3]);
	for (int i = 1; i < numberOfStandardParameters; i++)
	{
		sum += uR[i] * splineSums[i+2];
	}
...
void BulkOnlySplinesOrigin::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	double sum = 0;
	double interval;
	int bin;
	...
	sum += uR[0] * (bcFactors[0][0] * splineSumsNew[0] + bcFactors[0][1] * splineSumsNew[1] + bcFactors[0][2] * splineSumsNew[2] + bcFactors[0][3]);
	for (int i = 1; i < numberOfStandardParameters; i++)
	{
		sum += uR[i] * splineSumsNew[i+2];
	}
...
double BulkOnlySplinesOrigin::CalculateWFQuotient(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
...
void BulkOnlySplinesOrigin::AcceptMove()
{
	wf = wfNew;
