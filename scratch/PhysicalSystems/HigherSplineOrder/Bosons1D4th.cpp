...
Bosons1D4th::Bosons1D4th(vector<double>& params, string configDirectory) :
...
void Bosons1D4th::SetNodes(vector<double> n)
{
	this->nodes = n;
	int periodicNodeCount = 4;
...
void Bosons1D4th::InitSystem()
{
	...
	//INFO: BC
	numberOfSplines = nodes.size() - 4 - 1; //4th order splines -(d+1)
	halfLength = LBOX / 2.0;
	maxDistance = nodes[nodes.size() - 5];

	if (nodes.empty())
	{
		for (int i = -4; i < N_PARAM + 4; i++)
		{
			nodes.push_back((i * halfLength) / ((double) N_PARAM - 1));
		}
	}
	splineWeights = SplineFactory::GetWeights(nodes, 4);

	//cut off
	SplineFactory::SetBoundaryConditions4_1D_OR_1(nodes, bcFactorsStart);
	SplineFactory::SetBoundaryConditions4_1D_CO_2(nodes, bcFactorsEnd, maxDistance);
	...
	if (N_PARAM != numberOfSplines - (4 - numberOfSpecialParametersStart) - (4 - numberOfSpecialParametersEnd))
	{
	...
	pairDistribution.InitGrid(0.0, nodes[nodes.size() - 5], nodes[nodes.size() - 5] / 100.0);
...
void Bosons1D4th::RefreshLocalOperators()
{
	//INFO: BC
	for (int i = 0; i < np1; i++)
	{
		this->localOperators[i] = (bcFactorsStart[i][0] * splineSums[0] + bcFactorsStart[i][1] * splineSums[1] + bcFactorsStart[i][2] * splineSums[2] + bcFactorsStart[i][3] * splineSums[3]);
	}
	int spIdx = 4;
	...
	{
		this->localOperators[i] = (bcFactorsEnd[idx][0] * splineSums[numberOfSplines - 4] + bcFactorsEnd[idx][1] * splineSums[numberOfSplines - 3] + bcFactorsEnd[idx][2] * splineSums[numberOfSplines - 2] + bcFactorsEnd[idx][3] * splineSums[numberOfSplines - 1]);
		idx++;
	}
}

void Bosons1D4th::CalculateLocalOperators(vector<vector<double> >& R)
{
	int bin;
	double rni;
	double rni2;
	...
	for (int n = 0; n < N; n++)
	{
		for (int i = 0; i < n; i++)
		{
			...
			if (rni < maxDistance)
			{
				...
				for (int p = 0; p < 5; p++)
				{
					splineSums[bin - p] += splineWeights[bin - p][p][0] + splineWeights[bin - p][p][1] * rni + splineWeights[bin - p][p][2] * rni2 + splineWeights[bin - p][p][3] * rni3 + splineWeights[bin - p][p][4] * rni4;
				}
			}
			else
			{
...
void Bosons1D4th::CalculateOtherLocalOperators(vector<vector<double> >& R)
{
	int s = 5;
	vector<double> vecrni(DIM);
		...
		for (int i = 0; i < N; i++)
		{
			...
			if (rni < maxDistance)
			{
				...
				if (i != n) //TODO: also include in (i < n) branch -> then only loop over i < n in "for (int i = 0; i < N; i++)"
				{
					...
					{
						//first derivative
						tmp1[4 - p] = splineWeights[bin - p][p][1] + 2.0 * splineWeights[bin - p][p][2] * rni + 3.0 * splineWeights[bin - p][p][3] * rni2 + 4.0 * splineWeights[bin - p][p][4] * rni3;
						//second derivative
						tmp2[4 - p] = 2.0 * splineWeights[bin - p][p][2] + 6.0 * splineWeights[bin - p][p][3] * rni + 12.0 * splineWeights[bin - p][p][4] * rni2;
					}
					...
					for (int a = 0; a < DIM; a++)
					{
						for (int b = 0; b < s; b++)
						{
							splineSumsD[bin - b][n][a] += tmp1[4 - b] * evecrni[a] * firstDerivativeFactor;
						}
					}
					double secondDerivativeFactor = DIM - 1.0; //INFO: at least correct for 2D and 3D (and 1D?)
					for (int b = 0; b < s; b++)
					{
						splineSumsD2[bin - b][n] += tmp2[4 - b] + secondDerivativeFactor / rni * tmp1[4 - b];
					}
				}
			}
		}
...
vector<double> Bosons1D4th::GetCenterOfMass(vector<vector<double> >& R)
{
	vector<double> com = { 0.0, 0.0, 0.0 };
	return com;
}

double Bosons1D4th::GetExternalPotential(vector<double>& r)
{
	return 0;
}

void Bosons1D4th::CalculateExpectationValues(vector<double>& O, vector<vector<vector<double> > >& sD, vector<vector<double> >& sD2, vector<double>& otherO, vector<double>& gr, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double tmp = 0;
	double kineticR = 0;
	double kineticI = 0;
	...
	for (int n = 0; n < N; n++)
	{
		...
		for (int i = 0; i < np1; i++)
		{
			for (int a = 0; a < DIM; a++)
			{
				tmp = (bcFactorsStart[i][0] * sD[0][n][a] + bcFactorsStart[i][1] * sD[1][n][a] + bcFactorsStart[i][2] * sD[2][n][a] + bcFactorsStart[i][3] * sD[3][n][a]);
				vecKineticSumR1[a] += uR[i] * tmp;
				vecKineticSumI1[a] += uI[i] * tmp;
			}
			tmp = (bcFactorsStart[i][0] * sD2[0][n] + bcFactorsStart[i][1] * sD2[1][n] + bcFactorsStart[i][2] * sD2[2][n] + bcFactorsStart[i][3] * sD2[3][n]);
		...
		for (int i = np2; i < np3; i++)
		{
			for (int a = 0; a < DIM; a++)
			{
				tmp = (bcFactorsEnd[idx][0] * sD[numberOfSplines - 4][n][a] + bcFactorsEnd[idx][1] * sD[numberOfSplines - 3][n][a] + bcFactorsEnd[idx][2] * sD[numberOfSplines - 2][n][a] + bcFactorsEnd[idx][3] * sD[numberOfSplines - 1][n][a]);
				vecKineticSumR1[a] += uR[i] * tmp;
				vecKineticSumI1[a] += uI[i] * tmp;
			}
			tmp = (bcFactorsEnd[idx][0] * sD2[numberOfSplines - 4][n] + bcFactorsEnd[idx][1] * sD2[numberOfSplines - 3][n] + bcFactorsEnd[idx][2] * sD2[numberOfSplines - 2][n] + bcFactorsEnd[idx][3] * sD2[numberOfSplines - 1][n]);
			kineticSumR2 += uR[i] * tmp;
			kineticSumI2 += uI[i] * tmp;
			idx++;
		}
...
void Bosons1D4th::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	RefreshLocalOperators();
...
void Bosons1D4th::CalculateExpectationValues(ICorrelatedSamplingData* sample, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
...
void Bosons1D4th::CalculateAdditionalSystemProperties(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
...
void Bosons1D4th::CalculateWavefunction(vector<double>& O, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double sum = 0;
...
void Bosons1D4th::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	CalculateLocalOperators(R);
	CalculateWavefunction(this->localOperators, uR, uI, phiR, phiI);
}

void Bosons1D4th::CalculateWavefunction(ICorrelatedSamplingData* sample, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	CSDataBulkSplines* s = dynamic_cast<CSDataBulkSplines*>(sample);
	CalculateWavefunction(s->localOperators, uR, uI, phiR, phiI);
}

void Bosons1D4th::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	double sum = 0;
	int bin;
	double rni;
	...
	for (int i = 0; i < N; i++)
	{
		if (i != changedParticleIndex)
		{
			...
			if (rni < maxDistance)
			{
				...
				for (int p = 0; p < 5; p++)
				{
					sumOldPerBin[bin - p] += splineWeights[bin - p][p][0] + splineWeights[bin - p][p][1] * rni + splineWeights[bin - p][p][2] * rni2 + splineWeights[bin - p][p][3] * rni3 + splineWeights[bin - p][p][4] * rni4;
				}
			}
			else
			{
			...
			if (rni < maxDistance)
			{
				...
				for (int p = 0; p < 5; p++)
				{
					sumNewPerBin[bin - p] += splineWeights[bin - p][p][0] + splineWeights[bin - p][p][1] * rni + splineWeights[bin - p][p][2] * rni2 + splineWeights[bin - p][p][3] * rni3 + splineWeights[bin - p][p][4] * rni4;
				}
			}
			else
			{
	...
	for (int i = 0; i < np1; i++)
	{
		sum +=  uR[i] * (bcFactorsStart[i][0] * splineSumsNew[0] + bcFactorsStart[i][1] * splineSumsNew[1] + bcFactorsStart[i][2] * splineSumsNew[2] + bcFactorsStart[i][3] * splineSumsNew[3]);
	}
	int spIdx = 4;
	for (int i = np1; i < np2; i++)
	{
	...
	for (int i = np2; i < np3; i++)
	{
		sum +=  uR[i] * (bcFactorsEnd[idx][0] * splineSumsNew[numberOfSplines - 4] + bcFactorsEnd[idx][1] * splineSumsNew[numberOfSplines - 3] + bcFactorsEnd[idx][2] * splineSumsNew[numberOfSplines - 2] + bcFactorsEnd[idx][3] * splineSumsNew[numberOfSplines - 1]);
		idx++;
	}
...
double Bosons1D4th::CalculateWFQuotient(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
...
void Bosons1D4th::InitCorrelatedSamplingData(vector<ICorrelatedSamplingData*>& data)
{
...
void Bosons1D4th::FillCorrelatedSamplingData(ICorrelatedSamplingData* data)
{
