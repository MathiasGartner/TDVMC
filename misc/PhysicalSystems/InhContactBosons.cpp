...
void InhContactBosons::RefreshLocalOperators()
{
	...
	offset = spf.np3;
	this->localOperators[offset + 0] * pc.splineSums[1];
	this->localOperators[offset + 1] * (pc.splineSums[0] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / this->latestParametersR[offset + 1]) + pc.splineSums[2]);
	spIdx = 3;
	...
	for (int n = 0; n < N; n++)
	{
		...
		for (int i = 0; i < N; i++)
		{
			...
			if (rni <= maxDistance)
			{
				...
				if (i < n)
				{
					//INFO: no potential for contact interaction
				}
...
void InhContactBosons::CalculateExpectationValues(vector<double>& O, vector<vector<vector<double> > >& spf_sD, vector<vector<double> >& spf_sD2, vector<vector<vector<double> > >& pc_sD, vector<vector<double> >& pc_sD2, vector<double>& otherO, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	for (int n = 0; n < N; n++)
	{
		...
		{
			tmp = pc_sD[1][n][a];
			vecKineticSumR1[a] += uR[offset + 0] * tmp;
			vecKineticSumI1[a] += uI[offset + 0] * tmp;
		}
		tmp = pc_sD2[1][n];
		kineticSumR2 += uR[offset + 0] * tmp;
		kineticSumI2 += uI[offset + 0] * tmp;
		for (int a = 0; a < DIM; a++)
		{
			tmp = (pc_sD[0][n][a] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / uR[offset + 1]) + pc_sD[2][n][a]);
			vecKineticSumR1[a] += uR[offset + 1] * tmp;			
			tmp = (pc_sD[0][n][a] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / uI[offset + 1]) + pc_sD[2][n][a]);
			vecKineticSumI1[a] += uI[offset + 1] * tmp;
		}
		tmp = (pc_sD2[0][n] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / uR[offset + 1]) + pc_sD2[2][n]);
		kineticSumR2 += uR[offset + 1] * tmp;
		tmp = (pc_sD2[0][n] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / uI[offset + 1]) + pc_sD2[2][n]);
		kineticSumI2 += uI[offset + 1] * tmp;
...
void InhContactBosons::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	...
	//INFO: PairCorrelation
	offset = spf.np3;
	sum += uR[offset + 0] * pc.splineSumsNew[1];
	sum += uR[offset + 1] * (pc.splineSumsNew[0] * (1.0 - 2.0 * params[1] * pc.nodeSpacing / uR[offset + 1]) + pc.splineSumsNew[2]);
	spIdx = 3;
	for (int i = pc.np1; i < pc.np2; i++)
	{
