...
BosonClusterWithLogParam::BosonClusterWithLogParam(vector<double>& params, string configDirectory) :
		BosonClusterWithLog(params, configDirectory)
{
}

void BosonClusterWithLogParam::InitSystem()
{
	BosonClusterWithLog::InitSystem();
...
void BosonClusterWithLogParam::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double temp;
	...
	for (int n = 0; n < N; n++)
	{
		...
		for (int a = 0; a < DIM; a++)
		{
			...
			vecKineticSumR1[a] += uR[N_PARAM - 4] * temp;
			vecKineticSumI1[a] += uI[N_PARAM - 4] * temp;
		...
		kineticSumR2 += uR[N_PARAM - 4] * temp;
		kineticSumI2 += uI[N_PARAM - 4] * temp;
		...
		for (int a = 0; a < DIM; a++)
		{
			vecKineticSumR1[a] += uR[N_PARAM - 1] * logSumD[n][a];
			vecKineticSumI1[a] += uI[N_PARAM - 1] * logSumD[n][a];
		}
		kineticSumR2 += uR[N_PARAM - 1] * logSumD2[n];
		kineticSumI2 += uI[N_PARAM - 1] * logSumD2[n];
	...
	for (int i = 2; i < N_PARAM - 4; i++)
	{
		localOperators[i] = splineSums[i + 1];
	}
	localOperators[N_PARAM - 4] = splineSums[numberOfSplines - 3] + bcFactors[2][0] * splineSums[numberOfSplines - 2] + bcFactors[2][1] * splineSums[numberOfSplines - 1];
	localOperators[N_PARAM - 3] = constSum + bcFactors[3][0] * splineSums[numberOfSplines - 2] + bcFactors[3][1] * splineSums[numberOfSplines - 1];
	localOperators[N_PARAM - 2] = linearSum + bcFactors[4][0] * splineSums[numberOfSplines - 2] + bcFactors[4][1] * splineSums[numberOfSplines - 1];
	localOperators[N_PARAM - 1] = logSum;
...
void BosonClusterWithLogParam::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double sum = 0;
	int bin;
	double rni;
	...
	//a(n-2)
	sum += uR[N_PARAM - 4] * (splineSums[numberOfSplines - 3] + bcFactors[2][0] * splineSums[numberOfSplines - 2] + bcFactors[2][1] * splineSums[numberOfSplines - 1]);
	//ac
	sum += uR[N_PARAM - 3] * (constSum + bcFactors[3][0] * splineSums[numberOfSplines - 2] + bcFactors[3][1] * splineSums[numberOfSplines - 1]);
	//alin
	sum += uR[N_PARAM - 2] * (linearSum + bcFactors[4][0] * splineSums[numberOfSplines - 2] + bcFactors[4][1] * splineSums[numberOfSplines - 1]);

	sum += uR[N_PARAM - 1] * logSum;
...
void BosonClusterWithLogParam::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	double sum = 0;
	...
	//a(n-2)
	sum += uR[N_PARAM - 4] * (splineSumsNew[numberOfSplines - 3] + bcFactors[2][0] * splineSumsNew[numberOfSplines - 2] + bcFactors[2][1] * splineSumsNew[numberOfSplines - 1]);
	//ac
	sum += uR[N_PARAM - 3] * (constSumNew + bcFactors[3][0] * splineSumsNew[numberOfSplines - 2] + bcFactors[3][1] * splineSumsNew[numberOfSplines - 1]);
	//alin
	sum += uR[N_PARAM - 2] * (linearSumNew + bcFactors[4][0] * splineSumsNew[numberOfSplines - 2] + bcFactors[4][1] * splineSumsNew[numberOfSplines - 1]);

	sum += uR[N_PARAM - 1] * logSumNew;
