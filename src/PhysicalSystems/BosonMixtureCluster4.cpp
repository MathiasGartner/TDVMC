...
void BosonMixtureCluster::InitSystem()
{
	...
	//TODO!!! what??
	int standardNumberOfParams = 28;
	int standardIndexrijSplit = 4;
	int standardIndexrijTail = 5;
	index = this->correlationIndexMapping[He4][He4];
	if (index > -1)
	{
		CorrelationFunctionData& cfd = this->corrFuncData[index];
		cfd.mcMillanFactor = -4.7;
		cfd.numberOfSplines = standardNumberOfParams; //as in calculations performed for Split X_4 Cluster (BosonClusterWithLogParam)
...
void BosonMixtureCluster::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	for (int n = 0; n < N; n++)
	{
			...
			//if (rni < maxDistance)
			{
				...
				if (i != n) //TODO: also include in (i < n) branch -> then only loop over i < n
				{
					...
					else
					{
						...
						{
							//first derivative
							tmp1[s - 1 - p] = cfd.splineWeights[bin - p][p][1] + 2.0 * cfd.splineWeights[bin - p][p][2] * rni + 3.0 * cfd.splineWeights[bin - p][p][3] * rni2 + 4.0 * cfd.splineWeights[bin - p][p][4] * rni3;
							//second derivative
							tmp2[s - 1 - p] = 2.0 * cfd.splineWeights[bin - p][p][2] + 6.0 * cfd.splineWeights[bin - p][p][3] * rni + 12.0 * cfd.splineWeights[bin - p][p][4] * rni2;
						}
						for (int a = 0; a < DIM; a++)
						{
							for (int b = 0; b < s; b++)
							{
								cfd.splineSumsD[bin - b][n][a] += tmp1[s - 1 - b] * evecrni[a];
							}
						}
						for (int b = 0; b < s; b++)
						{
							double secondDerivativeFactor = DIM - 1.0; //INFO: at least correct for 2D and 3D (and 1D?)
							cfd.splineSumsD2[bin - b][n] += tmp2[s - 1 - b] + secondDerivativeFactor / rni * tmp1[s - 1 - b];
						}
					}
					for (int a = 0; a < DIM; a++)
					{
		...
		for (auto& cfd : this->corrFuncData)
		{
			...
			for (int a = 0; a < DIM; a++)
			{
				temp = (cfd.mcMillanSumD[n][a] + cfd.bcFactors[0][0] * cfd.splineSumsD[0][n][a] + cfd.bcFactors[0][1] * cfd.splineSumsD[1][n][a] + cfd.bcFactors[0][2] * cfd.splineSumsD[2][n][a]);
				opd.vecKineticSumR1[a] += uR[0 + offset] * temp;
				opd.vecKineticSumI1[a] += uI[0 + offset] * temp;
				temp = (cfd.splineSumsD[3][n][a] + cfd.bcFactors[1][0] * cfd.splineSumsD[0][n][a] + cfd.bcFactors[1][1] * cfd.splineSumsD[1][n][a] + cfd.bcFactors[1][2] * cfd.splineSumsD[2][n][a]);
				opd.vecKineticSumR1[a] += uR[1 + offset] * temp;
				opd.vecKineticSumI1[a] += uI[1 + offset] * temp;
			}
			temp = (cfd.mcMillanSumD2[n] + cfd.bcFactors[0][0] * cfd.splineSumsD2[0][n] + cfd.bcFactors[0][1] * cfd.splineSumsD2[1][n] + cfd.bcFactors[0][2] * cfd.splineSumsD2[2][n]);
			opd.kineticSumR2 += uR[0 + offset] * temp;
			opd.kineticSumI2 += uI[0 + offset] * temp;
			temp = (cfd.splineSumsD2[3][n] + cfd.bcFactors[1][0] * cfd.splineSumsD2[0][n] + cfd.bcFactors[1][1] * cfd.splineSumsD2[1][n] + cfd.bcFactors[1][2] * cfd.splineSumsD2[2][n]);
			...
			for (int k = 2; k < paramOffset - 4; k++)
			{
				for (int a = 0; a < DIM; a++)
				{
					opd.vecKineticSumR1[a] += uR[k + offset] * cfd.splineSumsD[k + 2][n][a];
					opd.vecKineticSumI1[a] += uI[k + offset] * cfd.splineSumsD[k + 2][n][a];
				}
				opd.kineticSumR2 += uR[k + offset] * cfd.splineSumsD2[k + 2][n];
				opd.kineticSumI2 += uI[k + offset] * cfd.splineSumsD2[k + 2][n];
			}

			for (int a = 0; a < DIM; a++)
			{
				temp = (cfd.splineSumsD[cfd.numberOfSplines - 4][n][a] + cfd.bcFactors[2][0] * cfd.splineSumsD[cfd.numberOfSplines - 3][n][a] + cfd.bcFactors[2][1] * cfd.splineSumsD[cfd.numberOfSplines - 2][n][a] + cfd.bcFactors[2][2] * cfd.splineSumsD[cfd.numberOfSplines - 1][n][a]);
				opd.vecKineticSumR1[a] += uR[paramOffset - 4 + offset] * temp;
				opd.vecKineticSumI1[a] += uI[paramOffset - 4 + offset] * temp;
				temp = (cfd.constSumD[n][a] + cfd.bcFactors[3][0] * cfd.splineSumsD[cfd.numberOfSplines - 3][n][a] + cfd.bcFactors[3][1] * cfd.splineSumsD[cfd.numberOfSplines - 2][n][a] + cfd.bcFactors[3][2] * cfd.splineSumsD[cfd.numberOfSplines - 1][n][a]);
				opd.vecKineticSumR1[a] += uR[paramOffset - 3 + offset] * temp;
				opd.vecKineticSumI1[a] += uI[paramOffset - 3 + offset] * temp;
				temp = (cfd.linearSumD[n][a] + cfd.bcFactors[4][0] * cfd.splineSumsD[cfd.numberOfSplines - 3][n][a] + cfd.bcFactors[4][1] * cfd.splineSumsD[cfd.numberOfSplines - 2][n][a] + cfd.bcFactors[4][2] * cfd.splineSumsD[cfd.numberOfSplines - 1][n][a]);
				opd.vecKineticSumR1[a] += uR[paramOffset - 2 + offset] * temp;
				opd.vecKineticSumI1[a] += uI[paramOffset - 2 + offset] * temp;
			}
			temp = (cfd.splineSumsD2[cfd.numberOfSplines - 4][n] + cfd.bcFactors[2][0] * cfd.splineSumsD2[cfd.numberOfSplines - 3][n] + cfd.bcFactors[2][1] * cfd.splineSumsD2[cfd.numberOfSplines - 2][n] + cfd.bcFactors[2][2] * cfd.splineSumsD2[cfd.numberOfSplines - 1][n]);
			opd.kineticSumR2 += uR[paramOffset - 4 + offset] * temp;
			opd.kineticSumI2 += uI[paramOffset - 4 + offset] * temp;
			temp = (cfd.constSumD2[n] + cfd.bcFactors[3][0] * cfd.splineSumsD2[cfd.numberOfSplines - 3][n] + cfd.bcFactors[3][1] * cfd.splineSumsD2[cfd.numberOfSplines - 2][n] + cfd.bcFactors[3][2] * cfd.splineSumsD2[cfd.numberOfSplines - 1][n]);
			opd.kineticSumR2 += uR[paramOffset - 3 + offset] * temp;
			opd.kineticSumI2 += uI[paramOffset - 3 + offset] * temp;
			temp = (cfd.linearSumD2[n] + cfd.bcFactors[4][0] * cfd.splineSumsD2[cfd.numberOfSplines - 3][n] + cfd.bcFactors[4][1] * cfd.splineSumsD2[cfd.numberOfSplines - 2][n] + cfd.bcFactors[4][2] * cfd.splineSumsD2[cfd.numberOfSplines - 1][n]);
	...
	{
		offset = nCFD * paramOffset;
		localOperators[0 + offset] = cfd.mcMillanSum + cfd.bcFactors[0][0] * cfd.splineSums[0] + cfd.bcFactors[0][1] * cfd.splineSums[1] + cfd.bcFactors[0][2] * cfd.splineSums[2];
		localOperators[1 + offset] = cfd.splineSums[3] + cfd.bcFactors[1][0] * cfd.splineSums[0] + cfd.bcFactors[1][1] * cfd.splineSums[1] + cfd.bcFactors[1][2] * cfd.splineSums[2];
		for (int i = 2; i < paramOffset - 4; i++)
		{
			localOperators[i + offset] = cfd.splineSums[i + 2];
		}
		localOperators[paramOffset - 4 + offset] = cfd.splineSums[cfd.numberOfSplines - 4] + cfd.bcFactors[2][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[2][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[2][2] * cfd.splineSums[cfd.numberOfSplines - 1];
		localOperators[paramOffset - 3 + offset] = cfd.constSum + cfd.bcFactors[3][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[3][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[3][2] * cfd.splineSums[cfd.numberOfSplines - 1];
		localOperators[paramOffset - 2 + offset] = cfd.linearSum + cfd.bcFactors[4][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[4][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[4][2] * cfd.splineSums[cfd.numberOfSplines - 1];
...
void BosonMixtureCluster::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	for (int n = 0; n < N; n++)
	{
		for (int i = 0; i < n; i++)
		{
			...
			else
			{
				...
				for (int p = 0; p < 5; p++)
				{
					cfd.splineSums[bin - p] += cfd.splineWeights[bin - p][p][0] + cfd.splineWeights[bin - p][p][1] * rni + cfd.splineWeights[bin - p][p][2] * rni2 + cfd.splineWeights[bin - p][p][3] * rni3 + cfd.splineWeights[bin - p][p][4] * rni4;
				}
			}
			cfd.logSum += log(rni);
		}
	...
	for (auto& cfd : this->corrFuncData)
	{
		offset = nCFD * paramOffset;
		//am
		sum += uR[0 + offset] * (cfd.mcMillanSum + cfd.bcFactors[0][0] * cfd.splineSums[0] + cfd.bcFactors[0][1] * cfd.splineSums[1] + cfd.bcFactors[0][2] * cfd.splineSums[2]);
		//a2
		sum += uR[1 + offset] * (cfd.splineSums[3] + cfd.bcFactors[1][0] * cfd.splineSums[0] + cfd.bcFactors[1][1] * cfd.splineSums[1] + cfd.bcFactors[1][2] * cfd.splineSums[2]);
		for (int i = 2; i < paramOffset - 4; i++)
		{
			sum += uR[i + offset] * cfd.splineSums[i + 2];
		}
		//a(n-3)
		sum += uR[paramOffset - 4 + offset] * (cfd.splineSums[cfd.numberOfSplines - 4] + cfd.bcFactors[2][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[2][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[2][2] * cfd.splineSums[cfd.numberOfSplines - 1]);
		//ac
		sum += uR[paramOffset - 3 + offset] * (cfd.constSum + cfd.bcFactors[3][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[3][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[3][2] * cfd.splineSums[cfd.numberOfSplines - 1]);
		//alin
		sum += uR[paramOffset - 2 + offset] * (cfd.linearSum + cfd.bcFactors[4][0] * cfd.splineSums[cfd.numberOfSplines - 3] + cfd.bcFactors[4][1] * cfd.splineSums[cfd.numberOfSplines - 2] + cfd.bcFactors[4][2] * cfd.splineSums[cfd.numberOfSplines - 1]);
...
void BosonMixtureCluster::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	...
	for (int i = 0; i < N; i++)
	{
		if (i != changedParticleIndex)
		{
			...
			else
			{
				...
				for (int p = 0; p < 5; p++)
				{
					cfd.sumOldPerBin[bin - p] += cfd.splineWeights[bin - p][p][0] + cfd.splineWeights[bin - p][p][1] * rni + cfd.splineWeights[bin - p][p][2] * rni2 + cfd.splineWeights[bin - p][p][3] * rni3 + cfd.splineWeights[bin - p][p][4] * rni4;
				}
			}
			cfd.logOld += log(rni);
	...
	for (auto& cfd : this->corrFuncData)
	{
		offset = nCFD * paramOffset;
		//am
		sum += uR[0 + offset] * (cfd.mcMillanSumNew + cfd.bcFactors[0][0] * cfd.splineSumsNew[0] + cfd.bcFactors[0][1] * cfd.splineSumsNew[1] + cfd.bcFactors[0][2] * cfd.splineSumsNew[2]);
		//a2
		sum += uR[1 + offset] * (cfd.splineSumsNew[3] + cfd.bcFactors[1][0] * cfd.splineSumsNew[0] + cfd.bcFactors[1][1] * cfd.splineSumsNew[1] + cfd.bcFactors[1][2] * cfd.splineSumsNew[2]);
		for (int i = 2; i < paramOffset - 4; i++)
		{
			sum += uR[i + offset] * cfd.splineSumsNew[i + 2];
		}
		//a(n-3)
		sum += uR[paramOffset - 4 + offset] * (cfd.splineSumsNew[cfd.numberOfSplines - 4] + cfd.bcFactors[2][0] * cfd.splineSumsNew[cfd.numberOfSplines - 3] + cfd.bcFactors[2][1] * cfd.splineSumsNew[cfd.numberOfSplines - 2] + cfd.bcFactors[2][2] * cfd.splineSumsNew[cfd.numberOfSplines - 1]);
		//ac
		sum += uR[paramOffset - 3 + offset] * (cfd.constSumNew + cfd.bcFactors[3][0] * cfd.splineSumsNew[cfd.numberOfSplines - 3] + cfd.bcFactors[3][1] * cfd.splineSumsNew[cfd.numberOfSplines - 2] + cfd.bcFactors[3][2] * cfd.splineSumsNew[cfd.numberOfSplines - 1]);
		//alin
		sum += uR[paramOffset - 2 + offset] * (cfd.linearSumNew + cfd.bcFactors[4][0] * cfd.splineSumsNew[cfd.numberOfSplines - 3] + cfd.bcFactors[4][1] * cfd.splineSumsNew[cfd.numberOfSplines - 2] + cfd.bcFactors[4][2] * cfd.splineSumsNew[cfd.numberOfSplines - 1]);
