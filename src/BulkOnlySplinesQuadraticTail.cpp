...
BulkOnlySplinesQuadraticTail::BulkOnlySplinesQuadraticTail(string configDirectory) : IPhysicalSystem()
{
...
void BulkOnlySplinesQuadraticTail::InitSystem()
{
	grBinCount = 100;
	grBinStartIndex = 3;
	numOfkValues = 300;
	...
	//cut off
	a13 = (2.0 * nodePointSpacing2 + 2.0 * nodePointSpacing * rijSplit + rijSplit2) / rijSplit4;
	a14 = -(nodePointSpacing2 - rijSplit2) / rijSplit4;
	a15 = (2.0 * nodePointSpacing2 - 2.0 * nodePointSpacing * rijSplit + rijSplit2) / rijSplit4;
	al = 0.0;//-1.0 / (maxDistance * maxDistance);
...
vector<double> BulkOnlySplinesQuadraticTail::GetCenterOfMass(vector<vector<double> >& R)
{
	vector<double> com = {0.0, 0.0, 0.0};
	return com;
}

void BulkOnlySplinesQuadraticTail::CalculateExpectationValues(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	...
	for (int n = 0; n < N; n++)
	{
		...
		for (int a = 0; a < DIM; a++)
		{
			temp = (quadraticSumD[n][a] + a13 * splineSumsD[numberOfSplines - 3][n][a] + a14 * splineSumsD[numberOfSplines - 2][n][a] + a15 * splineSumsD[numberOfSplines - 1][n][a]);
			vecKineticSumR1[a] += uR[N_PARAM - 1] * temp;
			vecKineticSumI1[a] += uI[N_PARAM - 1] * temp;
		}
		temp = (quadraticSumD2[n] + a13 * splineSumsD2[numberOfSplines - 3][n] + a14 * splineSumsD2[numberOfSplines - 2][n] + a15 * splineSumsD2[numberOfSplines - 1][n]);
	...
	localOperators[N_PARAM - 1] = (quadraticSum + al + a13 * splineSums[numberOfSplines - 3] + a14 * splineSums[numberOfSplines - 2] + a15 * splineSums[numberOfSplines - 1]);
...
void BulkOnlySplinesQuadraticTail::CalculateAdditionalSystemProperties(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
    ClearVector(additionalSystemProperties);
...
void BulkOnlySplinesQuadraticTail::CalculateWavefunction(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI)
{
	double sum = 0;
	double interval;
	int bin;
	...
	sum += uR[N_PARAM - 1] * (quadraticSum + al + a13 * splineSums[numberOfSplines - 3] + a14 * splineSums[numberOfSplines - 2] + a15 * splineSums[numberOfSplines - 1]);

	wf = exp(sum + phiR);
	exponent = sum;
}

void BulkOnlySplinesQuadraticTail::CalculateWFChange(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
	double sum = 0;
	...
	sum += uR[N_PARAM - 1] * (quadraticSumNew + al + a13 * splineSumsNew[numberOfSplines - 3] + a14 * splineSumsNew[numberOfSplines - 2] + a15 * splineSumsNew[numberOfSplines - 1]);

	wfNew = exp(sum + phiR);
	exponentNew = sum;
}

double BulkOnlySplinesQuadraticTail::CalculateWFQuotient(vector<vector<double> >& R, vector<double>& uR, vector<double>& uI, double phiR, double phiI, int changedParticleIndex, vector<double>& oldPosition)
{
...
void BulkOnlySplinesQuadraticTail::AcceptMove()
{
	wf = wfNew;
